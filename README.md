...
####	Because our laptops have 2 network routes - one to company's network (10.* ip address) and one to engineering lap (ip address 192.168.*), while Scailx camera is connected to engineering network (ip address 192.168.*), in VirtualBox we need to replace the rtsp line in the python program hostname with ip address of our Scailx camera, for example
...
#### Latest updates with new python programs to create gst rtsp server from Scailx camera with settings from pipeline json file (same syntax as Scailx Portal ;-).
...
#### 6. On Scailx device terminal, run

`python3 gst_rtsp_stream.py -h`    (Show help.)

`python3 gst_rtsp_stream.py -p data/settings/camera0_pipeline.json`    
(Load camera settings and server net url from a pipeline file.)

`python3 gst_rtsp_stream.py -p data/settings/camera0_pipeline.json -t 3001`    
(Load camera settings and specify port number to override pipeline file.)

`python3 gst_rtsp_stream.py -p data/settings/camera0_pipeline.json -t 0`    
(Load camera settings and use random assigned port number available.)
...
`python opencv_stream.py --p ../data/settings/camera0_pipeline.json`     
(Load same pipeline file to access server url.)

`python opencv_stream.py --p ../data/settings/camera0_pipeline.json -t 3001`     
(Load same pipeline file to access server url with port updated.)

`python opencv_stream.py --p ../data/settings/camera0_port3001_pipeline.json`     
(Load the pipeline file generated by Scailx program above to access live server url.)

#### Remember that in order to use newly saved pipeline files from Scailx device, we need to scp them to Windows. 
#### For example:
